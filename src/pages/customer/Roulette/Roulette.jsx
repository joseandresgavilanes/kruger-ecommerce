...
export const Roulette = () => {
  ...
  const getCouponsPrizes = async () => {
    ...
    setCoupons(
      filtered.filter(
        (coupon) => coupon.status === "NOT_USED" && coupon.userId === null
      )
    );
    setIsLoading(false);
  };
  ...
  return isLoading ? (
    ...
    <>
      ...
      >
        <div className="flex align-items-center flex-column pt-6 px-3">
          ...
          <img
            ...
            src="https://i.pinimg.com/originals/29/8f/2c/298f2ce6dead72fe992253c5ba0eba95.gif"
            alt="price roulette"
          />
          <h3 style={{ marginTop: 30 }}>Cupón de bienvenida activado!</h3>
          <h2 style={{ marginTop: 20 }}>
            <b>
              Haz activado tu cupón de descuento de bienvenida, úsalo en tus
              próximas compras.{" "}
            </b>
          </h2>
          ...
          <h2 style={{ lineHeight: 1.5, textIndent: "1rem", marginTop: 20 }}>
            <b>Cantidad: </b> {coupons[winPrizeIndex].quantity}{" "}
            {coupons[winPrizeIndex].type === "DIRECT" ? "$" : "%"}
          </h2>
        </div>
      </Dialog>
      ...
      >
        <Button
          style={{ width: "50%", justifyContent: "center", marginTop: "30px" }}
          onClick={handleStart}
          label="OBTEN TU CUPÓN GRATIS !"
