...
const Coupon = ({ onCouponActivated }) => {
  ...
  async function validarCupon() {
    ...
    if (resp && resp.status !== "USED") {
      ...
      setMessage("El cupón ha sido activado!");
      ...
      setMessage(
        !resp
          ? "Cupon is not valid!"
          : `Cupon is already ${resp.status.toLowerCase()}!`
      );
      setShow(true);
  ...
  return (
    <div>
      <div className="coupon-container">
        <h4>Tienes cupón? Ingrésalo aqui!</h4>
        <input
          type="text"
          ...
          ref={inputRef}
        />
        <button onClick={validarCupon} ref={btnRef} className="validar-btn">
          Validar
        </button>
      </div>
